...
class DPDT(Component):
    ...
    Hardware note on physical lever direction
    -----------------------------------------
    The direction the LEVER physically points for each throw depends on the
    specific switch model — some toggles have "lever up = top contacts
    closed", many others have the inverse because the actuator pivots
    opposite the visible flag. Because this varies, the diagram and truth
    tables generated by this class refer to positions by **THROW A / OFF /
    THROW B** (not "up / middle / down"). The installer orients their
    specific switch to match their preferred lever-direction → mode mapping.
    """
    ...
    def draw(self, svg):
        ...
        # ON-OFF-ON visual cue: a small dashed rectangle BELOW the switch
        # body labelled "OFF = both poles float" so the reader immediately
        # sees the kill-in-middle behavior without having to parse the
        # truth table. (Labels go BELOW the body to stay clear of wires
        # that approach the switch from either side.)
        if self.throws == 3:
            tag_y = self.y + self.HEIGHT / 2 + 10
            svg.append(dw.Rectangle(
                self.x - self.WIDTH / 2 - 2, tag_y - 6,
                self.WIDTH + 4, 13,
                fill="none", stroke="#888", stroke_width=0.8,
                stroke_dasharray="3,2", rx=2,
            ))
            svg.append(dw.Text(
                "OFF = both poles float",
                7, self.x, tag_y + 2.5,
                text_anchor="middle", font_family="sans-serif",
